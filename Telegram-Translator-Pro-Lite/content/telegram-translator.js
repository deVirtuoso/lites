...
async function loadSettings() {
  ...
  document.documentElement.style.setProperty("--ttl-color", settings.color)
}
...
function shouldSkipText(text) {
  ...
  if (/^(edited|forwarded|reply|view|views|today|yesterday)$/i.test(value)) return true
  return false
}

function getMessageText(el) {
  const clone = el.cloneNode(true)
  clone.querySelectorAll(".ttl-translation, .ttl-divider, .time, .message-time").forEach((node) => node.remove())
  return clone.textContent.trim()
}

function isLikelyMessage(el, text) {
  if (shouldSkipText(text)) return false
  if (el.closest(".sidebar, .chatlist, .left-column, .topbar, .composer-wrapper")) return false
  return Boolean(el.closest(".message, .bubble"))
}

function getMessageContainer(el) {
  return el.closest(".message, .bubble") || el.parentElement
}

function clearTranslation(container) {
  container.querySelectorAll(":scope > .ttl-translation, :scope > .ttl-divider").forEach((el) => el.remove())
  container.querySelectorAll('[data-ttl-origin="true"]').forEach((el) => {
    el.removeAttribute("data-ttl-origin")
    el.removeAttribute("data-ttl-hidden-origin")
  })
  delete container.dataset.ttlDone
  delete container.dataset.ttlText
}

function setBilingualDisplay(container) {
  const origin = container.querySelector('[data-ttl-origin="true"]')
  const translation = container.querySelector(":scope > .ttl-translation")
  container.querySelectorAll(":scope > .ttl-divider").forEach((el) => el.remove())
  if (!origin || !translation) return

  if (settings.bilingualDisplay) {
    origin.removeAttribute("data-ttl-hidden-origin")
    const divider = document.createElement("hr")
    divider.className = "ttl-divider"
    container.insertBefore(divider, translation)
  } else {
    origin.setAttribute("data-ttl-hidden-origin", "true")
  }
}

function refreshTranslatedMessages() {
  document.querySelectorAll('[data-ttl-done="true"]').forEach(setBilingualDisplay)
...
function collectMessages() {
  ...
  document.querySelectorAll(MESSAGE_SELECTOR).forEach((messageEl) => {
    ...
    if (container.dataset.ttlDone === "true" || container.dataset.ttlDone === "pending") return

    container.dataset.ttlDone = "pending"
    container.dataset.ttlText = text
...
function applyTranslations(items, translations) {
  items.forEach((item, index) => {
    ...
    if (!translated || translated === item.text) {
      ...
      item.container.dataset.ttlDone = "true"
      return
    }

    item.messageEl.setAttribute("data-ttl-origin", "true")

    const translation = document.createElement("div")
    translation.className = "ttl-translation"
...
function ensureFloatingToggle() {
  ...
  floatingToggle.className = "ttl-floating-toggle"
...
function getComposerWrapper(composer) {
  return (
    composer.closest(".input-message-container, .composer-wrapper, .rows-wrapper-wrapper") ||
...
function ensureComposeButton() {
  ...
  composeButton.className = "ttl-compose-button"
